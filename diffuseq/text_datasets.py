...
def helper_tokenize(sentence_lst, vocab_dict, seq_len):
    ...
    padded_ids = _collate_batch_helper(split_data['input_ids'], vocab_dict.pad_token_id, seq_len)
    padded_mask = _collate_batch_helper(split_data['input_mask'], 1, seq_len)
    lm_datasets = Dataset2.from_dict(
        {'input_ids': padded_ids, 'input_mask': padded_mask, 'target_len': split_data['target_len']},
    )
